...
public class CQTCrudController extends AbstractController{
	private static final Logger log = LogManager.getLogger(CQTCrudController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		String header = "Thêm mới cơ quan thuế";
		...
		switch (transaction) {
		case "ql-cqt-cre":
			header = "Thêm mới cơ quan thuế";
			action = "CREATE";
			isEdit = true;
			break;
		case "ql-cqt-edit":
			header = "Thay đổi thông tin cơ quan thuế";
			action = "EDIT";
			isEdit = true;
			break;
		case "ql-cqt-detail":
			header = "Chi tiết cơ quan thuế";
			action = "DETAIL";
			isEdit = false;
			break;
		...
		if("|ql-cqt-edit|ql-cqt-detail".indexOf(transaction) != -1)
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String action) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/ql-cqt/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			JsonNode jsonData = Json.serializer().nodeFromObject(rsp.getObjData());
			req.setAttribute("Code", commons.getTextJsonNode(jsonData.at("/code")));
			req.setAttribute("Codett", commons.getTextJsonNode(jsonData.at("/tinhthanh_ma")));
	...
	public BaseDTO checkDataToAccept(HttpServletRequest req, HttpSession session, String transaction
		...
		codett = commons.getParameterFromRequest(req, "codett");
		name = commons.getParameterFromRequest(req, "name");
			...
			case "ql-cqt-edit":	
				if("".equals(code)) {
					dto.setErrorCode(1);
					dto.getErrorMessages().add("Mã cơ quan thuế không được để trống.");
				}
				if("".equals(codett)) {
					dto.setErrorCode(1);
					dto.getErrorMessages().add("Mã thỉnh thành không được để trống.");
				}
				if("".equals(name)) {
					dto.setErrorCode(1);
	...
	public BaseDTO execCheckDataToSave(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn cơ quan thuế?";
		switch (transaction) {
		case "ql-cqt-cre":
			messageConfirm = "Bạn có muốn thêm mới cơ quan thuế không?";
			break;
		case "ql-cqt-edit":
			messageConfirm = "Bạn có muốn thay đổi thông tin cơ quan thuế không?";
			break;
		default:
			dto = new BaseDTO();
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "ql-cqt-cre": actionCode = Constants.MSG_ACTION_CODE.CREATED; break;
		case "ql-cqt-edit": actionCode = Constants.MSG_ACTION_CODE.MODIFY; break;
		default:
			dtoRes = new BaseDTO();
		...
		hData.put("_id", _id);
		hData.put("code", code);
		hData.put("codett", codett);
		hData.put("name", name);
		MsgRsp rsp = restAPI.callAPINormal("/ql-cqt/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
