...
@RequestMapping({
	"/agent-del",
	"/agent-send-cqt",
	"/agent-send-emailauto"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class AgentActionController extends AbstractController{
	private static final Logger log = LogManager.getLogger(AgentActionController.class);
	...
	public BaseDTO checkData(HttpServletRequest req, HttpSession session, String transaction
		...
		if("".equals(_id)) {
			dto.setErrorCode(1);
			dto.getErrorMessages().add("Không tìm thấy thông tin phiếu.");
		}
	...
	public BaseDTO execCheckData(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn xóa phiếu xuất kho này không?";
		switch (transaction) {
		case "agent-del":
			messageConfirm = "Bạn có muốn xóa phiếu xuất kho này không?";
			break;
		case "agent-send-cqt":
	...
	public BaseDTO checkDataToSend(HttpServletRequest req, HttpSession session, String transaction
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			String _tmp = "";
			title = ii.getTaxCode() + " " + ii.getName() + " Thông báo phát hành hóa phiếu xuất kho hàng gửi bán đại lý";
	...
	public BaseDTO execSendMail(HttpServletRequest req, HttpSession session
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/agent/send-mail", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			dtoRes.setErrorCode(0);
