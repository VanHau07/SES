...
@Controller
@RequestMapping("/ql-tinh")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class TinhThanhController extends AbstractController{
	private static final Logger log = LogManager.getLogger(TinhThanhController.class);
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Danh sách tỉnh thành");
		CurrentUserProfile cup = getCurrentlyAuthenticatedPrincipal();
		LoadParameter(cup, locale, req, "DETAIL");
		return "setting-admin/tinhthanh";
	}
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/ql-tinh/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
