...
public class ExportCRUDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(ExportCRUDController.class);
...
//					hItem.put(commons.getTextJsonNode(o.get("VND")), commons.getTextJsonNode(o.get("USD")));	
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		req.setAttribute("THDon", "PHIẾU XUẤT KHO KIÊM VẬN CHUYỂN NỘI BỘ ĐIỆN TỬ");
		
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/export/getMS", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		String mauso = "";
		if(rspStatus.getErrorCode() == 0) {		
			mauso =  rsp.getResponseStatus().getErrorDesc();
		}
		if("export-cre".equals(transaction) && !"init-dc-tt".equals(method)) {
			if(!mauso.equals("")) {
		...
		String header = "Thêm mới phiếu xuất kho kiêm vận chuyển nội bộ";
		...
		switch (transaction) {
		case "export-cre":
			header = "Thêm mới phiếu xuất kho kiêm vận chuyển nội bộ";
			action = "CREATE";
			isEdit = true;
			break;
		case "export-edit":
			header = "Thay đổi thông tin phiếu xuất kho kiêm vận chuyển nội bộ";
			action = "EDIT";
			isEdit = true;
			break;
		case "export-detail":
			header = "Chi tiết phiếu xuất kho kiêm vận chuyển nội bộ";
			action = "DETAIL";
			isEdit = false;
			break;
		case "export-copy":
			header = "Copy phiếu xuất kho kiêm vận chuyển nội bộ";
			action = "EDIT";
			isEdit = true;
			break;
		case "export-sign":
			header = "Ký phiếu xuất kho kiêm vận chuyển nội bộ";
			action = "SIGN";
			isEdit = false;
			break;
		...
		if("|export-edit|export-detail|export-copy|export-sign|".indexOf(transaction) != -1
		...
		if("|export-cre|export-edit|export-copy|".indexOf(transaction) != -1)
			LoadParameter(cup, locale, req, action);

		if(!"".equals(errorDesc))
			req.setAttribute("messageError", errorDesc);
			
		return "export/export-crud";
	}
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action, String transaction, String method) throws Exception{
		if("".equals(_id)) {
			errorCode = "NOT FOUND";
			errorDesc = "Không tìm thấy thông tin phiếu xuất kho kiêm vận chuyển nội bộ.";
			return;
		}
		BaseDTO baseDTO = new BaseDTO(req);
		Msg msg = baseDTO.createMsg(cup, Constants.MSG_ACTION_CODE.INQUIRY);
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			String TCTBao = "";
			String notice = "";
			if("init-dc-tt".equals(method) && "export-cre".equals(transaction)) {
		...
		req.setAttribute("MCCQT", commons.getTextJsonNode(jsonData.at("/MCCQT")));
			...
			req.setAttribute("NBanLDDNBo", commons.getTextJsonNode(jsonData.at("/EInvoiceDetail/NDHDon/NBan/LDDNBo")));
			...
			req.setAttribute("NBanHDS", commons.getTextJsonNode(jsonData.at("/EInvoiceDetail/NDHDon/NBan/HDSo")));
			...
			req.setAttribute("NMuaTDV", commons.getTextJsonNode(jsonData.at("/EInvoiceDetail/NDHDon/NMua/NMuaTDV")));
			req.setAttribute("NMuaMST", commons.getTextJsonNode(jsonData.at("/EInvoiceDetail/NDHDon/NMua/MST")));
...
//			req.setAttribute("DSHHDVu", Json.serializer().toString(prds));
...
//		chkXuatTheoLoaiTienTt = commons.getParameterFromRequest(req, "chk-xuat-theo-loai-tien-tt").replaceAll("\\s", "");
		...
		khHDS = commons.getParameterFromRequest(req, "kh-hds").trim().replaceAll("\\s+", " ");
	
		LDDNBo = commons.getParameterFromRequest(req, "kh-lenh-dd-noi-bo").trim().replaceAll("\\s+", " ");
		...
		khMST = commons.getParameterFromRequest(req, "kh-mst").trim().replaceAll("\\s+", " ");
		khTDV = commons.getParameterFromRequest(req, "kh-tdv");
		...
		if("export-edit".equals(transaction)) {
			if("".equals(_id)) {
				dto.setErrorCode(1);
...
//			}
			...
			if("".equals(tienBangChu)) {
				...
				dto.setErrorCode(1);
				dto.getErrorMessages().add("Lệnh điều động nội bộ không được trống.");
			}if("".equals(khDiaChi)) {
				dto.setErrorCode(1);
				dto.getErrorMessages().add("Địa chỉ nhập kho không được trống.");
			}if("".equals(Dchi)) {
				dto.setErrorCode(1);
				dto.getErrorMessages().add("Địa chỉ xuất kho không được trống.");
			}if("".equals(PTVChuyen)) {
				dto.setErrorCode(1);
				dto.getErrorMessages().add("Phương tiện vận chuyển không được trống.");
			}
			if(!khMst.equals("")) {
	...
	public BaseDTO execCheckDataToSave(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn thêm mới phiếu xuất kho kiêm vận chuyển nội bộ không?";
		switch (transaction) {
		case "export-cre":
			messageConfirm = "Bạn có muốn thêm mới phiếu xuất kho kiêm vận chuyển nội bộ không?";
			break;
		case "export-copy":
			messageConfirm = "Bạn có muốn thêm mới phiếu xuất kho kiêm vận chuyển nội bộ không?";
			break;
		case "export-edit":
			messageConfirm = "Bạn có muốn thay đổi thông tin phiếu xuất kho kiêm vận chuyển nội bộ không?";
			break;
		case "export-deleteAll":
		...
		if(null == jsonNodeTmp || jsonNodeTmp.size() == 0) {
			dto.setErrorCode(999);
			dto.setResponseData("Phiếu xuất kho kiêm vận chuyển nội bộ chưa có dữ liệu hàng hóa.");
			return dto;
		}
...
//		}
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		if(null == jsonNodeTmp || jsonNodeTmp.size() == 0) {
			dtoRes.setErrorCode(999);
			dtoRes.setResponseData("Phiếu xuất kho kiêm vận chuyển nội bộ chưa có dữ liệu hàng hóa.");
			return dtoRes;
		}
		...
		switch (transaction) {
		case "export-cre": actionCode = Constants.MSG_ACTION_CODE.CREATED; break;
		case "export-edit": actionCode = Constants.MSG_ACTION_CODE.MODIFY; break;
		case "export-copy": actionCode = Constants.MSG_ACTION_CODE.COPY; break;
		case "export-deleteAll": actionCode = Constants.MSG_ACTION_CODE.DELETEALL; break;
		default:
			dtoRes = new BaseDTO();
		...
		hData.put("KhMST", khMST);
		hData.put("KhTDV", khTDV);
		hData.put("KhSoDt", khSoDt);
		hData.put("KhHDS", khHDS);
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/export/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			dtoRes.setErrorCode(0);
			switch (transaction) {
			case "export-cre":
				dtoRes.setResponseData("Thêm mới thông tin phiếu xuất kho kiêm vận chuyển nội bộ thành công.");
				break;
			case "export-copy":
				dtoRes.setResponseData("Thêm mới thông tin phiếu xuất kho kiêm vận chuyển nội bộ thành công.");
				break;
			case "export-edit":
				dtoRes.setResponseData("Cập nhật thông tin phiếu xuất kho kiêm vận chuyển nội bộ thành công.");
				break;
			default:
	...
	public BaseDTO execCheckDataToSign(Locale locale, HttpServletRequest req, HttpSession session
		...
		if("".equals(_id)) {
			dto.setErrorCode(1);
			dto.setResponseData("Không tìm thấy phiếu xuất kho kiêm vận chuyển nội bộ cần ký.");
			return dto;
		}
		...
		FileInfo fileInfo = restAPI.callAPIGetFileInfo("/export/get-file-for-sign", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if(null == fileInfo.getContentFile()) {
			dto.setErrorCode(999);
			dto.setResponseData("Không tìm thấy dữ liệu phiếu xuất kho kiêm vận chuyển nội bộ.");
			return dto;
		}
		
		//CHECK SHDON CO HOP LE HAY KHONG
		MsgRsp rsp = restAPI.callAPINormal("/export/check_shd", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() != 0) {			
			dto.setErrorCode(999);
...
//		FileCopyUtils.copy(multipartFile.getBytes(), Paths.get(SystemParams.DIR_TMP_SAVE_FILES, "tmp.xml").toFile());
		...
		contentDisposition = ContentDisposition.builder("form-data").name("XMLFileSigned").filename("export-signed.xml").build();
		...
		if (result.getStatusCode() == org.springframework.http.HttpStatus.OK) {
			...
			dtoRes = new BaseDTO(result.getStatusCode().value(), "Thực hiện ký phiếu xuất kho kiêm vận chuyển nội bộ không thành công.");
		}
		return dtoRes;
	}
	...
	public String history(Locale locale, HttpServletRequest req, HttpSession session
		...
		boolean isEdit = false;
		if("|export-history|".indexOf(transaction) != -1)
	...
	private void inquiryhistory(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action) throws Exception{
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/export/history/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
	...
	private void inquirychange(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session,
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/export/change/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			MsgPage page = rsp.getMsgPage();
			grid.setTotal(1);
	...
	public BaseDTO execCheckMST(Locale locale, HttpServletRequest req, HttpSession session
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/export/check_mst", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
	...
	public BaseDTO execSaveNMua(Locale locale, HttpServletRequest req, HttpSession session
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/export/save_nmua", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {					
			dto.setErrorCode(0);	
			return dto;
	...
	public BaseDTO execCheckHistoryMST(Locale locale, HttpServletRequest req, HttpSession session
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/export/check_history_mst", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			String TH = rsp.getResponseStatus().getErrorDesc();
