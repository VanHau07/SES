...
public class LHDCRUDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(LHDCRUDController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		String header = "Thêm mới loại hóa đơn";
		...
		switch (transaction) {
		case "ql-lhd-cre":
			header = "Thêm mới loại hóa đơn";
			action = "CREATE";
			isEdit = true;
			break;
		case "ql-lhd-edit":
			header = "Thay đổi thông tin loại hóa đơn";
			action = "EDIT";
			isEdit = true;
			break;
		case "ql-lhd-detail":
			header = "Chi tiết loại hóa đơn";
			action = "DETAIL";
			isEdit = false;
			break;
		...
		if("|ql-lhd-edit|ql-lhd-detail".indexOf(transaction) != -1)
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String action) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/ql-lhd/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
	...
	public BaseDTO execCheckDataToSave(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn thêm loại hóa đơn?";
		switch (transaction) {
		case "ql-lhd-cre":
			messageConfirm = "Bạn có muốn thêm mới loại hóa đơn không?";
			break;
		case "ql-lhd-edit":
			messageConfirm = "Bạn có muốn thay đổi thông tin loại hóa đơn không?";
			break;
		default:
			dto = new BaseDTO();
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "ql-lhd-cre": actionCode = Constants.MSG_ACTION_CODE.CREATED; break;
		case "ql-lhd-edit": actionCode = Constants.MSG_ACTION_CODE.MODIFY; break;
		default:
			dtoRes = new BaseDTO();
		...
		hData.put("code", code);
		hData.put("name", name);
		MsgRsp rsp = restAPI.callAPINormal("/ql-lhd/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
