...
@Controller
@RequestMapping("/ql-user-check")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class QLUserCheckController extends AbstractController{
	private static final Logger log = LogManager.getLogger(QLUserCheckController.class);
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Danh sách tài khoản user check");
		return "user_check/qlUserCheck";
	}
...
//		hData.put("Email", email);
		...
		MsgRsp rsp = restAPI.callAPINormal("/qlUserCheckAdmin/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					hItem.put("NameIssuer", commons.getTextJsonNode(row.at("/NameIssuer")));
					...
					hItem.put("NgLap", commons.getTextJsonNode(row.at("/CreateUserFullName")));
