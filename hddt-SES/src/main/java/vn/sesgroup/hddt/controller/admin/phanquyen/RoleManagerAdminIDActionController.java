...
@RequestMapping({
	"/RolesManageAdminDel"
	,"/RolesManageAdminActive"
	, "/RolesManageAdminDeActive"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class RoleManagerAdminIDActionController extends AbstractController{
	private static final Logger log = LogManager.getLogger(RoleManagerAdminIDActionController.class);
	...
	public BaseDTO execCheckData(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn xóa nhóm phân quyền này không?";
		switch (transaction) {
		case "RolesManageAdminDel":
			messageConfirm = "Bạn có muốn xóa nhóm phân quyền này không?";
			break;
		case "RolesManageAdminActive":
			messageConfirm = "Bạn có muốn kích hoạt nhóm phân quyền không?";
			break;
		case "RolesManageAdminDeActive":
			messageConfirm = "Bạn có muốn hủy kích hoạt nhóm phân quyền này không?";
			break;
		default:
			dto = new BaseDTO();
	...
	public BaseDTO execData(HttpServletRequest req, HttpSession session
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			switch (transaction) {
			case "RolesManageAdminDel":
				dtoRes.setResponseData("Xóa thông tin nhóm phân quyền thành công.");
				break;
			case "RolesManageAdminActive":
