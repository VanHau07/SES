...
@Controller
@RequestMapping("/dsnmua")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class DSNMuaController extends AbstractController{
	private static final Logger log = LogManager.getLogger(DSNMuaController.class);
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Quản lý khách hàng");
		
		CurrentUserProfile cup = getCurrentlyAuthenticatedPrincipal();
		//req.setAttribute("USERS-CHECK", "USER-CHECK");
		return "user_check/dsnmua";
	}
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/dsnmua/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					hItem.put("DChi", commons.getTextJsonNode(row.at("/DChi")));
					hItem.put("SDThoai", commons.getTextJsonNode(row.at("/SDThoai")));
					
					
					hItem.put("DCTDTu", commons.getTextJsonNode(row.at("/DCTDTu")));
									
					if(!row.at("/NLap").equals("")) {
						hItem.put("NLap", 
...
//		tu_ngay = commons.getParameterFromRequest(req, "tu-ngay").replaceAll("\\s", "");
//		den_ngay = commons.getParameterFromRequest(req, "den-ngay").trim().replaceAll("\\s+", "");
//		if("".equals(tu_ngay)) {
//			dto.setErrorCode(1);
//			dto.getErrorMessages().add("Vui lòng chọn từ ngày.");
//		}else if(!commons.checkLocalDate(tu_ngay, Constants.FORMAT_DATE.FORMAT_DATE_WEB)) {
//			dto.setErrorCode(1);
//			dto.getErrorMessages().add("Ngày không đúng định dạng.");
//		}
//		if("".equals(den_ngay)) {
//			dto.setErrorCode(1);
//			dto.getErrorMessages().add("Vui lòng chọn ngày.");
//		}else if(!commons.checkLocalDate(den_ngay, Constants.FORMAT_DATE.FORMAT_DATE_WEB)) {
//			dto.setErrorCode(1);
//			dto.getErrorMessages().add("Ngày không đúng định dạng.");
//		}
	...
	public BaseDTO execCheckDataToSave(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm =  "Bạn có muốn thêm danh sách này không";
		switch (transaction) {
		case "dsnmua":
			messageConfirm = "Bạn có muốn thêm danh sách này không?";
			break;
	...
	public BaseDTO execRefreshStatusCQT(Locale locale, HttpServletRequest req, HttpSession session
		...
		HashMap<String, Object> hData = new HashMap<>();
//		hData.put("TuNgay", tu_ngay);
//		hData.put("DenNgay", den_ngay);
		msg.setObjData(hData);
		
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/dsnmua/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			dto.setErrorCode(0);
			dto.setResponseData("Lấy thông tin thành công.");
		}else {
...
//		hData.put("NmuaTen", nmuaTen);
	...
	public void exportExcel(Locale locale, HttpServletRequest req, HttpServletResponse resp, HttpSession session
		...
		try {
			...
			String url = "/dsnmua/export-excel";		
