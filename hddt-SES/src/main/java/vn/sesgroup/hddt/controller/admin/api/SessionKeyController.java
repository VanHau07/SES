...
@Controller
@RequestMapping("/session_key")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class SessionKeyController extends AbstractController{
	private static final Logger log = LogManager.getLogger(SessionKeyController.class);
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Set Session Key API");
		req.setAttribute("map_status", Constants.MAP_STATUS);
		return "api-admin/session_key";
	}
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/session_key/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					hItem.put("SessionKey", commons.getTextJsonNode(row.at("/SessionKey")));
