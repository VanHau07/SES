...
public class AgentImportController extends AbstractController{
	private static final Logger log = LogManager.getLogger(AgentImportController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		req.setAttribute("_header_", "Import hóa đơn PXk hàng bán ký gửi đại lý");
		LoadParameter(cup, locale, req, action);
		return "agent/agent-import";
	}
	...
	public BaseDTO execImportData(HttpServletRequest req, HttpSession session
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/agent/import-data", cup.getLoginRes().getToken(), HttpMethod.POST, root);
