...
@Controller
@RequestMapping({
	"/einvoice1-import-auto"

})

@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class EInvoice1ImportAutoController extends AbstractController{
	private static final Logger log = LogManager.getLogger(EInvoice1ImportAutoController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		req.setAttribute("_header_", "Import hóa đơn bán hàng");
		LoadParameter(cup, locale, req, action);
		return "einvoice1/einvoice-import-auto";
	}
	...
	public BaseDTO execImportData(HttpServletRequest req, HttpSession session
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/einvoice1/import-data-auto", cup.getLoginRes().getToken(), HttpMethod.POST, root);
