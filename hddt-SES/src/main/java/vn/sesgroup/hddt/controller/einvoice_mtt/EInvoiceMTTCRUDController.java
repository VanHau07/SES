...
@RequestMapping({
	...
	, "/einvoice_mtt-copy"
	, "/einvoice_mtt-sign"
	, "/einvoice_mtt-history"
	, "/einvoice_mtt-edit"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class EInvoiceMTTCRUDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(EInvoiceMTTCRUDController.class);
...
//					hItem.put(commons.getTextJsonNode(o.get("VND")), commons.getTextJsonNode(o.get("USD")));	
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		case "einvoice_mtt-cre":
			header = "Thêm mới hóa đơn điện tử từ máy tính tiền";
			action = "CREATE";
			isEdit = true;
			break;
		case "einvoice_mtt-edit":
			header = "Thay đổi thông tin hóa đơn điện tử từ máy tính tiền";
			action = "EDIT";
			isEdit = true;
			break;
		case "einvoice_mtt-detail":
			header = "Chi tiết hóa đơn điện tử từ máy tính tiền";
			action = "DETAIL";
			isEdit = false;
			break;
		case "einvoice_mtt-copy":
			header = "Copy hóa đơn điện tử từ máy tính tiền";
			action = "COPY";
			isEdit = true;
			break;
		case "einvoice_mtt-sign":
			header = "Ký hóa đơn điện tử từ máy tính tiền";
			action = "SIGN";
			isEdit = false;
			break;
		...
		return "einvoice_mtt/einvoice_mtt-crud";
	}
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action, String transaction, String method) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/einvoice_mtt/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			JsonNode jsonData = Json.serializer().nodeFromObject(rsp.getObjData());
			String TCTBao = "";
			String notice = "";
			if("init-dc-tt".equals(method) && "einvoice_mtt-cre".equals(transaction)) {
			...
			req.setAttribute("LoaiHDon", commons.getTextJsonNode(jsonData.at("/EInvoiceDetail/TTChung/THDon")));
			req.setAttribute("LoaiHDonCode", commons.getTextJsonNode(jsonData.at("/EInvoiceDetail/TTChung/LoaiHD")));
...
//			hR.put("", elemTmp.select(cssQuery))
	...
	@RequestMapping(value = "/check-data-sign-mtt",  produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.POST)
	@ResponseBody
	public BaseDTO execCheckDataToSignMTT(Locale locale, HttpServletRequest req, HttpSession session
			, @RequestAttribute(name = "transaction", value = "", required = false) String transaction) throws Exception {
		String token = "";
		if (null != session.getAttribute(Constants.SESSION_TYPE.SESSION_TOKEN_EXECUTE)) {
			token = session.getAttribute(Constants.SESSION_TYPE.SESSION_TOKEN_EXECUTE).toString();
		...
		FileInfo fileInfo = restAPI.callAPIGetFileInfo("/einvoice_mtt/get-file-for-sign-mtt", cup.getLoginRes().getToken(), HttpMethod.POST, root);
	...
	@RequestMapping(
			value = "/signFileMTT/{_id:.+}"
			, produces = MediaType.APPLICATION_JSON_VALUE
			, method = RequestMethod.POST
			, consumes = MediaType.MULTIPART_FORM_DATA_VALUE
		)
	@ResponseBody
	public BaseDTO processSignFileMTT(HttpServletRequest req, HttpSession session
			, @RequestAttribute(name = "transaction", value = "", required = false) String transaction
			, @RequestParam("XMLFileSigned") MultipartFile multipartFile
			, @RequestParam("certificate") String certificate
			, @PathVariable(name = "_id", required = true) String _id
			) throws Exception{
		BaseDTO dtoRes = new BaseDTO();
...
//		FileCopyUtils.copy(multipartFile.getBytes(), Paths.get(SystemParams.DIR_TMP_SAVE_FILES, "tmp.xml").toFile());
		...
		HttpEntity<MultiValueMap<String, Object>> requestBody = new HttpEntity<>(body, headers);
		String url = "/einvoice_mtt/sign-single-mtt";
		ResponseEntity<MsgRsp> result = restTemplate.exchange(APIParams.HTTP_URI + url, HttpMethod.POST, requestBody, MsgRsp.class);
		/*END - CONNECT TO API*/		
		if (result.getStatusCode() == org.springframework.http.HttpStatus.OK) {
			MsgRsp rsp = result.getBody();
	...
	@RequestMapping(value = "/check-data-signAll-mtt",  produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.POST)
	@ResponseBody
	public BaseDTO execCheckDataToSignAllMTT(Locale locale, HttpServletRequest req, HttpSession session
			, @RequestAttribute(name = "transaction", value = "", required = false) String transaction) throws Exception {
		String token = "";
		if (null != session.getAttribute(Constants.SESSION_TYPE.SESSION_TOKEN_EXECUTE)) {
			token = session.getAttribute(Constants.SESSION_TYPE.SESSION_TOKEN_EXECUTE).toString();
		...
		FileInfo fileInfo = restAPI.callAPIGetFileInfo("/einvoice_mtt/get-file-for-signAll-mtt", cup.getLoginRes().getToken(), HttpMethod.POST, root);
	...
	@RequestMapping(
			value = "/signAllFileMTT/{_id:.+}"
			, produces = MediaType.APPLICATION_JSON_VALUE
			, method = RequestMethod.POST
			, consumes = MediaType.MULTIPART_FORM_DATA_VALUE
		)
	@ResponseBody
	public BaseDTO processSignAllFileMTT(HttpServletRequest req, HttpSession session
			, @RequestAttribute(name = "transaction", value = "", required = false) String transaction
			, @RequestParam("XMLFileSigned") MultipartFile multipartFile
			, @RequestParam("certificate") String certificate
			, @PathVariable(name = "_id", required = true) String _id
...
//		FileCopyUtils.copy(multipartFile.getBytes(), Paths.get(SystemParams.DIR_TMP_SAVE_FILES, "tmp.xml").toFile());
		...
		HttpEntity<MultiValueMap<String, Object>> requestBody = new HttpEntity<>(body, headers);
		String url = "/einvoice_mtt/signAll-single-mtt";
		ResponseEntity<MsgRsp> result = restTemplate.exchange(APIParams.HTTP_URI + url, HttpMethod.POST, requestBody, MsgRsp.class);
		/*END - CONNECT TO API*/		
		if (result.getStatusCode() == org.springframework.http.HttpStatus.OK) {
			MsgRsp rsp = result.getBody();
