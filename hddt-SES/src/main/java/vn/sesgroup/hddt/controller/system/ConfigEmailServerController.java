...
@Controller
@RequestMapping("/config-email-server")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class ConfigEmailServerController extends AbstractController{
	private static final Logger log = LogManager.getLogger(ConfigEmailServerController.class);
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req
		...
		return "system/config-email-server";
	}
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String action) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/config-email-server/detail", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			String mailjet = jsonData.at("/MailJet").toString();	
			...
			if(mailjet.equals("")) {				
				...
				mail_ = mailjet.charAt(1);
			}
			char check_mail = (char) mail_;
			
			String c_mail=String.valueOf(check_mail);  
...
//		}
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/config-email-server/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
