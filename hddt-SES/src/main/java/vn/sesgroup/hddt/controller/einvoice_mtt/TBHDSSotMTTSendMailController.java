...
@Controller
@RequestMapping({
	"/tbhdssot_mtt-send-mail"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class TBHDSSotMTTSendMailController extends AbstractController{
	private static final Logger log = LogManager.getLogger(TBHDSSotMTTSendMailController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		return "einvoice_mtt/tbhdssot_mtt-send-mail";
	}
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action) throws Exception{
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			String _tmp = "";
			title = ii.getTaxCode() + " " + ii.getName() + " Thông báo hủy/giải trình của NNT Hóa Đơn điện tử có sai sót";
		...
		if(emailReceivemain!="" || emailReceivecc =="") {
			...
			sb.append("<p><span style='font-family: Times New Roman;font-size: 13px;'>" + ii.getName() + " xin trân trọng thông báo đến Quý Khách giải trình về việc hóa đơn điện tử có sai sót</span></p>\n");
	...
	public BaseDTO execSendMail(HttpServletRequest req, HttpSession session
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tbhdssot_mtt/send-mail", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
