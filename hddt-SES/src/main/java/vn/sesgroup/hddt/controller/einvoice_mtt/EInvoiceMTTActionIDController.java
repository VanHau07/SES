...
@RequestMapping({
	"/einvoice_mtt-del",
	"/einvoice_mtt-send-cqt",
	"/einvoice_mtt-send-cqtAll",
	"/einvoice_mtt-send-emailauto"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class EInvoiceMTTActionIDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(EInvoiceMTTActionIDController.class);
...
//				MsgRsp rsp1 = restAPI.callAPINormal("/einvoice_mtt/send_cqt_mtt", cup.getLoginRes().getToken(), HttpMethod.POST, root);
//				MspResponseStatus rspStatus1 = rsp.getResponseStatus();
...
//				}	
	...
	public BaseDTO execSendMail(HttpServletRequest req, HttpSession session
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/einvoice_mtt/send-mail", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			dtoRes.setErrorCode(0);
