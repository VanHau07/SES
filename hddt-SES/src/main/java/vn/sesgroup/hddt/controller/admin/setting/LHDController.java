...
@Controller
@RequestMapping("/ql-lhd")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class LHDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(LHDController.class);
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Danh sách loại hóa đơn");
		return "setting-admin/lhd";
	}
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/ql-lhd/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
