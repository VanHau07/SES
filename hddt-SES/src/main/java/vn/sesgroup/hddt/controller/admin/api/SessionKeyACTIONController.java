...
public class SessionKeyACTIONController extends AbstractController{
	private static final Logger log = LogManager.getLogger(SessionKeyACTIONController.class);
	...
	public BaseDTO checkData(HttpServletRequest req, HttpSession session, String transaction
		...
		if("".equals(_id)) {
			dto.setErrorCode(1);
			dto.getErrorMessages().add("Không tìm thấy thông tin thông tin.");
		}
	...
	public BaseDTO execCheckData(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn xóa thông tin này không?";
		switch (transaction) {
		case "session_key_del":
			messageConfirm = "Bạn có muốn xóa thông tin này không?";
			break;
		default:
			dto = new BaseDTO();
