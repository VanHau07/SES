...
@RequestMapping({
	...
	"/tbhdssot_mtt-detail",
	"/tbhdssot_mtt-edit",
	"/tbhdssot_mtt-history",
	"/tbhdssot_mtt-sign"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class TBHDSSotMTTCRUDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(TBHDSSotMTTCRUDController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		String header = "Tạo thông báo hóa đơn sai sót từ máy tính tiền";
		String action = "CREATE";
		boolean isEdit = false;
		
		switch (transaction) {
		case "tbhdssot_mtt-cre":
			header = "Tạo thông báo hóa đơn sai sót từ máy tính tiền";
			action = "CREATE";
			isEdit = true;
			break;
		case "tbhdssot_mtt-edit":
			header = "Thay đổi thông báo HĐ sai sót từ máy tính tiền";
			action = "EDIT";
			isEdit = true;
			break;
		case "tbhdssot_mtt-detail":
			header = "Chi tiết thông báo HĐ sai sót từ máy tính tiền";
			action = "DETAIL";
			isEdit = false;
			break;
		case "tbhdssot_mtt-sign":
			header = "Ký thông báo HĐ sai sót từ máy tính tiền";
			action = "SIGN";
			isEdit = false;
			break;
		default:
		...
		if("|tbhdssot_mtt-edit|tbhdssot_mtt-sign|tbhdssot_mtt-detail|".indexOf(transaction) != -1)
		...
		if("|tbhdssot_mtt-cre|tbhdssot_mtt-edit|".indexOf(transaction) != -1)
		...
		return "einvoice_mtt/tbhdssot_mtt-crud";
	}
	...
	private void inquiryhistory(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tbhdssot_mtt/history/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tbhdssot_mtt/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			JsonNode jsonData = Json.serializer().nodeFromObject(rsp.getObjData());
...
//			req.setAttribute("TThanhCode", commons.getTextJsonNode(jsonData.at("/TinhThanhInfo/code")));
...
//			req.setAttribute("CQThueName", commons.getTextJsonNode(jsonData.at("/ChiCucThueInfo/name")));
			...
			if(!jsonData.at("/DSHDon").isMissingNode()) {
				for(JsonNode o: jsonData.at("/DSHDon")) {
					...
					hItem.put("SHDon", commons.formatNumberBillInvoice(commons.getTextJsonNode(o.at("/SHDon"))));
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "tbhdssot_mtt-cre": actionCode = Constants.MSG_ACTION_CODE.CREATED; break;
		case "tbhdssot_mtt-edit": actionCode = Constants.MSG_ACTION_CODE.MODIFY; break;
		default:
			dtoRes = new BaseDTO();
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tbhdssot_mtt/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
	...
	public BaseDTO execCheckDataToSign(Locale locale, HttpServletRequest req, HttpSession session
		...
		if(!"tbhdssot_mtt-sign".equals(transaction)) {
			dto = new BaseDTO();
			dto.setErrorCode(998);
		...
		FileInfo fileInfo = restAPI.callAPIGetFileInfo("/tbhdssot_mtt/get-file-for-sign", cup.getLoginRes().getToken(), HttpMethod.POST, root);
...
//		FileCopyUtils.copy(multipartFile.getBytes(), Paths.get(SystemParams.DIR_TMP_SAVE_FILES, "tmp.xml").toFile());
		...
		String url = "/tbhdssot_mtt/sign-single";
