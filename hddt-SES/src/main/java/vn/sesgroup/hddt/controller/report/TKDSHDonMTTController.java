...
@Controller
@RequestMapping("/tkdsmtt")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class TKDSHDonMTTController extends AbstractController{
	private static final Logger log = LogManager.getLogger(TKDSHDonMTTController.class);
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Thống kê danh sách hóa đơn điện tử từ máy tính tiền");
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tkdsmtt/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
	...
	public void exportExcel(Locale locale, HttpServletRequest req, HttpServletResponse resp, HttpSession session
		...
		try {
			...
			String url = "/tkdsmtt/export-excel-to-fast";
			if("export-excel-general".equals(method))
				url = "/tkdsmtt/export-excel-general";
			else if("export-excel-dshdon-ctiet".equals(method))
				url = "/tkdsmtt/export-excel-dshdon-ctiet";
			vn.sesgroup.hddt.dto.FileInfo fileInfo = restAPI.callAPIGetFileInfo(url, cup.getLoginRes().getToken(), HttpMethod.POST, root);
			if(null != fileInfo) {
				String fileNameOut = "DANH-SACH-HD-MTT-DANG-SD.xlsx";
				if("export-excel-general".equals(method))
					fileNameOut = "Report-BKHD-MTT-General.xlsx";
				else if("export-excel-dshdon-ctiet".equals(method))
					fileNameOut = "chi-tiet-danh-sach-hoa-don-tu-may-tinh-tien.xlsx";
