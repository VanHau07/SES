...
@Controller
@RequestMapping("/email-send-hd")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class EmailSendHDon extends AbstractController {
	private static final Logger log = LogManager.getLogger(EmailSendHDon.class);
	@Autowired
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception {
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Danh sách email gửi hóa đơn");
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception {
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/email-send-hd/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if (rspStatus.getErrorCode() == 0) {
			...
			if (!jsonData.at("/rows").isMissingNode()) {
				...
				for (JsonNode row : rows) {
					...
					hItem.put("TenNBan", commons.getTextJsonNode(row.at("/EInvoiceDetail/NDHDon/NBan/Ten")));
					hItem.put("MSTNBan", commons.getTextJsonNode(row.at("/EInvoiceDetail/NDHDon/NBan/MST")));
					hItem.put("TenNMua", commons.getTextJsonNode(row.at("/EInvoiceDetail/NDHDon/NMua/Ten")));
					hItem.put("MSTNMua", commons.getTextJsonNode(row.at("/EInvoiceDetail/NDHDon/NMua/MST")));
					hItem.put("DCTDTuNMua", commons.getTextJsonNode(row.at("/EInvoiceDetail/NDHDon/NMua/DCTDTu")));
	...
	public void exportExcel(Locale locale, HttpServletRequest req, HttpServletResponse resp, HttpSession session
		...
		try {
			...
			String url = "/email-send-hd/export-excel";		
