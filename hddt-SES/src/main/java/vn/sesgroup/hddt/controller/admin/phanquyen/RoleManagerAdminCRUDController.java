...
@Controller
@RequestMapping({
	"/RolesManageAdminCreate"
	, "/RolesManageAdminDetail"
	, "/RolesManageAdminEdit"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class RoleManagerAdminCRUDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(RoleManagerAdminCRUDController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		case "RolesManageAdminDetail":
			...
			isEdit = false;
			break;
		case "RolesManageAdminActive":
			break;
		case "RolesManageAdminDeActive":	
			break;
		default:
			break;
		}
		
		if("|RolesManageAdminEdit|RolesManageAdminDetail|".indexOf(transaction) != -1
				|| "init-dc-tt".equals(method))
			inquiry(cup, locale, req, session, _id, action, transaction, method);
		if("RolesManageAdminCreate".equals(transaction)) {
		...
		if("|RolesManageAdminCreate|RolesManageAdminEdit|".indexOf(transaction) != -1)
		
		if(!"".equals(errorDesc))
			req.setAttribute("messageError", errorDesc);
		
		return "role-user-admin/roleManagerAdminCRUD";
	}
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action, String transaction, String method) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/roleManagerAdmin/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
	...
	public BaseDTO checkDataToAccept(HttpServletRequest req, HttpSession session, String transaction
		...
		if("RolesManageAdminEdit".equals(transaction)) {
			if("".equals(_id)) {
				dto.setErrorCode(1);
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "RolesManageAdminCreate": actionCode = Constants.MSG_ACTION_CODE.CREATED; break;
		case "RolesManageAdminEdit": actionCode = Constants.MSG_ACTION_CODE.MODIFY; break;
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/roleManagerAdmin/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
