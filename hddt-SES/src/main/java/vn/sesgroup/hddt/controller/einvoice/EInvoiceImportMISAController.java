...
@Controller
@RequestMapping({
	"/einvoice-import-misa"

})

@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class EInvoiceImportMISAController extends AbstractController{
	private static final Logger log = LogManager.getLogger(EInvoiceImportMISAController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		req.setAttribute("_header_", "Import hóa đơn VAT");
		LoadParameter(cup, locale, req, action);
		return "einvoice/einvoice-import-misa";
	}
	...
	@RequestMapping(value = "/import-misa",  produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.POST)
	@ResponseBody
	public BaseDTO execImportData(HttpServletRequest req, HttpSession session
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/einvoice/import-data-misa", cup.getLoginRes().getToken(), HttpMethod.POST, root);
