...
@Controller
@RequestMapping("/tkdshdon")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class TKDSHDonController extends AbstractController{
	private static final Logger log = LogManager.getLogger(TKDSHDonController.class);
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tkdshdon/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
	...
	public void exportExcel(Locale locale, HttpServletRequest req, HttpServletResponse resp, HttpSession session
		...
		try {
			...
			String url = "/tkdshdon/export-excel-to-fast";
			if("export-excel-general".equals(method))
				url = "/tkdshdon/export-excel-general";
			else if("export-excel-dshdon-ctiet".equals(method))
				url = "/tkdshdon/export-excel-dshdon-ctiet";
			vn.sesgroup.hddt.dto.FileInfo fileInfo = restAPI.callAPIGetFileInfo(url, cup.getLoginRes().getToken(), HttpMethod.POST, root);
			if(null != fileInfo) {
				String fileNameOut = "DANH-SACH-HD-DANG-SD.xlsx";
				if("export-excel-general".equals(method))
					fileNameOut = "Report-BKHD-General.xlsx";
				else if("export-excel-dshdon-ctiet".equals(method))
					fileNameOut = "chi-tiet-danh-sach-hoa-don.xlsx";
