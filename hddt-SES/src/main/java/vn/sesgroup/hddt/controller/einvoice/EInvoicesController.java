...
public class EInvoicesController extends AbstractController{
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Danh sách hóa đơn");
...
//						);
	...
	public BaseDTO execRefreshStatusCQT(Locale locale, HttpServletRequest req, HttpSession session
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/einvoice/refresh-status-cqt", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			dto.setErrorCode(0);
	...
	@RequestMapping(value = "/delete_HD",  produces = MediaType.APPLICATION_JSON_VALUE, method = RequestMethod.POST)
	  ...
	  public BaseDTO delete_HD(Locale locale, HttpServletRequest req, HttpSession session
	    ...
	    JSONRoot root = new JSONRoot(msg);
	    MsgRsp rsp = restAPI.callAPINormal("/einvoice/delete_HD", cup.getLoginRes().getToken(), HttpMethod.POST, root);
	    MspResponseStatus rspStatus = rsp.getResponseStatus();
	    if(rspStatus.getErrorCode() == 0) {
	      dto.setErrorCode(0);
