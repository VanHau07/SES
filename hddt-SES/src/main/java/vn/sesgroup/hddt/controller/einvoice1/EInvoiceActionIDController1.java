...
@RequestMapping({
	"/einvoice1-del",
	"/einvoice1-send-cqt",
	"/einvoice1-send-emailauto"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class EInvoiceActionIDController1 extends AbstractController{
	private static final Logger log = LogManager.getLogger(EInvoiceActionIDController1.class);
	...
	public BaseDTO checkDataToSend(HttpServletRequest req, HttpSession session, String transaction
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			String _tmp = "";
			title = ii.getTaxCode() + " " + ii.getName() + " Thông báo phát hành hóa đơn bán hàng";
	...
	public BaseDTO execSendMail(HttpServletRequest req, HttpSession session
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/einvoice1/send-mail", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			dtoRes.setErrorCode(0);
