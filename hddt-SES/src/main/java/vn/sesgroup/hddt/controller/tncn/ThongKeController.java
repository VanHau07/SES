...
@Controller
@RequestMapping("/tktncn")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class ThongKeController extends AbstractController{
	...
	@RequestMapping(value = "/init", method = {RequestMethod.POST, RequestMethod.GET})
	  public String init(Locale locale, Principal principal,HttpServletRequest req) throws Exception {
	    req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Thống kê chứng từ khấu trừu thuế thu nhập cá nhân");
	...
	@SuppressWarnings("unused")
	@RequestMapping(value = {"/viewReportPdf/{idata1}", "/exportDataExcel/{idata1}"},  method = RequestMethod.GET)
		...
		try {
			...
			String[] words = idata1.split("-");
	         ...
	         String method =  words[1];
				...
				boolean isXml = "exportDataExcel".equals(method);
			...
			if(isXml) {
				hDataDetail.put("typeExport", "excel");	
			}else {
			...
			FileInfo fileInfo = restAPI.callAPIGetFileInfo("/tktncn/viewReport", cup.getLoginRes().getToken(), HttpMethod.POST, root);
			...
			if(fileInfo!=null) {
				...
				if(isXml) {
					...
					resp.setHeader("Content-Disposition", "attachment; filename=" + "Thong-Ke-Chung-Tu-Khau-Tru-Thue.xlsx");
				}else {
					resp.setHeader("Content-Disposition", "inline; filename=" + "Thong-Ke-Chung-Tu-Khau-Tru-Thue.pdf");
				}
