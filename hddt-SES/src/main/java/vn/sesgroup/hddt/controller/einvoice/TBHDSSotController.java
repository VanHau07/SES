...
@Controller
@RequestMapping("/tbhdssot")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class TBHDSSotController extends AbstractController{
	private static final Logger log = LogManager.getLogger(TBHDSSotController.class);
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tbhdssot/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					hItem.put("TBLoi", commons.getTextJsonNode(row.at("/TBLoi")));
	...
	public BaseDTO execRefreshStatusCQT(Locale locale, HttpServletRequest req, HttpSession session
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tbhdssot/refresh-status-cqt", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			dto.setErrorCode(0);
