...
public class HTTKCrudController extends AbstractController{
	private static final Logger log = LogManager.getLogger(HTTKCrudController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		String header = "Thêm mới hình thức tờ khai";
		...
		switch (transaction) {
		case "ql-httk-cre":
			header = "Thêm mới hình thức tờ khai";
			action = "CREATE";
			isEdit = true;
			break;
		case "ql-httk-edit":
			header = "Thay đổi thông tin hình thức tờ khai";
			action = "EDIT";
			isEdit = true;
			break;
		case "ql-httk-detail":
			header = "Chi tiết hình thức tờ khai";
			action = "DETAIL";
			isEdit = false;
			break;
		...
		if("|ql-httk-edit|ql-httk-detail".indexOf(transaction) != -1)
			...
			return "setting-admin/httk-crud";
	}
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String action) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/ql-httk/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
	...
	public BaseDTO checkDataToAccept(HttpServletRequest req, HttpSession session, String transaction
		...
		name = commons.getParameterFromRequest(req, "name");
			...
			case "ql-httk-edit":	
				...
				if("".equals(code)) {
					dto.setErrorCode(1);
					dto.getErrorMessages().add("Mã không được để trống.");
				}
				break;
			default:
				break;
	...
	public BaseDTO execCheckDataToSave(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn thêm hình thức tờ khai?";
		switch (transaction) {
		case "ql-httk-cre":
			messageConfirm = "Bạn có muốn thêm mới hình thức tờ khai không?";
			break;
		case "ql-httk-edit":
			messageConfirm = "Bạn có muốn thay đổi thông tin hình thức tờ khai không?";
			break;
		default:
			dto = new BaseDTO();
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "ql-httk-cre": actionCode = Constants.MSG_ACTION_CODE.CREATED; break;
		case "ql-httk-edit": actionCode = Constants.MSG_ACTION_CODE.MODIFY; break;
		default:
			dtoRes = new BaseDTO();
		...
		hData.put("code", code);
		hData.put("name", name);
		MsgRsp rsp = restAPI.callAPINormal("/ql-httk/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
