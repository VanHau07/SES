...
public class ChungTuActionIDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(ChungTuActionIDController.class);
	...
	public BaseDTO execCheckData(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn xóa bỏ chứng từ này không?";
		switch (transaction) {
		case "cttncn-del":
			messageConfirm = "Bạn có muốn xóa bỏ chứng từ này không?";
			break;
	...
	public BaseDTO execData(HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "cttncn-del":
			actionCode = Constants.MSG_ACTION_CODE.XoaBo;
			break;
		default:
			dtoRes = new BaseDTO();
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			switch (transaction) {
			case "cttncn-del":
				dtoRes.setResponseData("Xóa bỏ thông tin chứng từ thành công.");
				break;
			default:
				break;
			}
