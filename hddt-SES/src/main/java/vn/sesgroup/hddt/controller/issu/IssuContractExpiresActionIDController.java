...
@Controller
@RequestMapping({
	"/issu-contract-expires-active",
	"/issu-contract-expires-deactive"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class IssuContractExpiresActionIDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(IssuContractExpiresActionIDController.class);
	...
	public BaseDTO execCheckData(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn thay đổi thông tin này không?";
		switch (transaction) {
	
		case "issu-contract-expires-active":
			messageConfirm = "Bạn có muốn thay đổi thông tin này không?";
			break;
		case "issu-contract-expires-deactive":
			messageConfirm = "Bạn có muốn thay đổi thông tin này không?";
			break;
		default:
			dto = new BaseDTO();
	...
	public BaseDTO execData(HttpServletRequest req, HttpSession session
		...
		String actionCode = Constants.MSG_ACTION_CODE.ACTIVE;
		switch (transaction) {
		case "issu-contract-expires-active":
		...
		MsgRsp rsp = restAPI.callAPINormal("/issu-contract-expires/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			case "issu-contract-expires-active":
				dtoRes.setResponseData("Thay đổi thành công.");
				break;
			case "issu-contract-expires-deactive":
				dtoRes.setResponseData("Thay đổi thành công.");
				break;
