...
public class ChungTuImportController extends AbstractController{
	private static final Logger log = LogManager.getLogger(ChungTuImportController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		req.setAttribute("_header_", "Import chứng từ TNCN");
		LoadParameter(cup, locale, req, action);
		return "tncn/cttncn-import";
	}
	...
	public BaseDTO execImportData(HttpServletRequest req, HttpSession session
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			dtoRes.setResponseData("Import thông tin thành công.");
		}else {
