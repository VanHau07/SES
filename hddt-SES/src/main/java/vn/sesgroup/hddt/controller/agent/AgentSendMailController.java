...
public class AgentSendMailController extends AbstractController{
	private static final Logger log = LogManager.getLogger(AgentSendMailController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		return "agent/agent-send-mai-agent";
	}
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action) throws Exception{
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			String _tmp = "";
			title = ii.getTaxCode() + " " + ii.getName() + " Thông báo phát hành phiếu xuất kho hàng gửi bán đại lý";
