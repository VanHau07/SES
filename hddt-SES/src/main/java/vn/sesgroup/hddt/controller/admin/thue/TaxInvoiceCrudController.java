...
@RequestMapping({
	...
	, "/tax_invoice-detail"
	,"/tax_invoice-active"
	,"/tax_invoice-deActive"
	,"/tax_invoice-default"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class TaxInvoiceCrudController extends AbstractController{
	private static final Logger log = LogManager.getLogger(TaxInvoiceCrudController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		String header = "Thêm mới thuế suất";
		...
		case "tax_invoice-cre":
			header = "Thêm mới thuế suất";
			action = "CREATE";
			isEdit = true;
			break;
		case "tax_invoice-edit":
			header = "Thay đổi thông tin thuế suất";
			action = "EDIT";
			isEdit = true;
			break;
		case "tax_invoice-detail":
			header = "Chi tiết thuế suất";
			action = "DETAIL";
			isEdit = false;
			break;
		case "tax_invoice-active":
			break;
		case "tax_invoice-deActive":	
			break;
		...
		if("|tax_invoice-edit|tax_invoice-detail|tax_invoice-active|tax_invoice-deActive|tax_invoice-default|".indexOf(transaction) != -1)
		...
		return "system/tax_invoice-crud";
	}
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String action) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/tax_invoice/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			req.setAttribute("STT", commons.getTextJsonNode(jsonData.at("/STT")));
			}else {
	...
	public BaseDTO checkDataToAccept(HttpServletRequest req, HttpSession session, String transaction
		...
		stt = commons.getParameterFromRequest(req, "stt");
...
//				}
	...
	public BaseDTO execCheckDataToSave(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn thêm mới thuế suất?";
		switch (transaction) {
		case "tax_invoice-cre":
			messageConfirm = "Bạn có muốn thêm mới thuế suất không?";
			break;
		case "tax_invoice-edit":
			messageConfirm = "Bạn có muốn thay đổi thông tin thuế suất không?";
			break;
		case "tax_invoice-active":
		...
		case "tax_invoice-deActive":
			...
			break;
		case "tax_invoice-default":
			messageConfirm = "Bạn có chắc chắn muốn Hủy/Set mặc định thuế suất này không?";
			break;
		case "tax_invoice-del":
	...
	public BaseDTO execSaveData(HttpServletRequest req, HttpSession session
		...
		switch (transaction) {
		case "tax_invoice-cre": actionCode = Constants.MSG_ACTION_CODE.CREATED; break;
		case "tax_invoice-edit": actionCode = Constants.MSG_ACTION_CODE.MODIFY; break;
		case "tax_invoice-active": actionCode = Constants.MSG_ACTION_CODE.ACTIVE; break;
		case "tax_invoice-deActive": actionCode = Constants.MSG_ACTION_CODE.DEACTIVE;break;
		case "tax_invoice-default": actionCode = Constants.MSG_ACTION_CODE.APPROVE;break;		
		case "tax_invoice-del": actionCode = Constants.MSG_ACTION_CODE.DELETE;break;		
		default:
			dtoRes = new BaseDTO();
		...
		hData.put("name", name);
		hData.put("stt", stt);
		MsgRsp rsp = restAPI.callAPINormal("/tax_invoice/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
			...
			case "tax_invoice-deActive":
				...
				break;
			case "tax_invoice-default":
				dtoRes.setResponseData("Hủy/Set mặc định thành công.");
				break;
			case "tax_invoice-del":
				dtoRes.setResponseData("Xóa thành công.");
				break;
			default:
