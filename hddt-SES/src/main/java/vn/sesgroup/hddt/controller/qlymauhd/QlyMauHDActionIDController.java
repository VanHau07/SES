...
public class QlyMauHDActionIDController extends AbstractController{
	private static final Logger log = LogManager.getLogger(QlyMauHDActionIDController.class);
	...
	public BaseDTO checkData(HttpServletRequest req, HttpSession session, String transaction
		...
		if("".equals(_id)) {
			dto.setErrorCode(1);
			dto.getErrorMessages().add("Không tìm thấy thông tin mẫu hóa đơn.");
		}
	...
	public BaseDTO execCheckData(Locale locale, HttpServletRequest req, HttpSession session
		...
		BaseDTO dto = new BaseDTO();
		String messageConfirm = "Bạn có muốn xóa mẫu hóa đơn này không?";
		switch (transaction) {
		case "qly-mauhd-del":
	...
	public BaseDTO execData(HttpServletRequest req, HttpSession session
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			switch (transaction) {
			case "qly-mauhd-del":
				dtoRes.setResponseData("Xóa thông tin mẫu hóa đơn thành công.");
				break;
			case "qly-mauhd-active":
