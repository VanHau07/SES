...
public class CreateUserAdminController extends AbstractController{
	private static final Logger log = LogManager.getLogger(CreateUserAdminController.class);
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Danh sách tài khoản admin");
		return "role-user-admin/createUserAdmin";
	}
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/createUserAdmin/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
