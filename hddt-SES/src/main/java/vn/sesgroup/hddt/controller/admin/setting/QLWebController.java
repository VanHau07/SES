...
@Controller
@RequestMapping("/ql-ttweb")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class QLWebController extends AbstractController{
	private static final Logger log = LogManager.getLogger(QLWebController.class);
	...
	private BaseDTO checkDataSearch(Locale locale, HttpServletRequest req, HttpSession session) {
		...
		phone = commons.getParameterFromRequest(req, "phone").replaceAll("\\s", "");
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		MsgRsp rsp = restAPI.callAPINormal("/ql-ttweb/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					hItem.put("Mail", commons.getTextJsonNode(row.at("/mail")));
