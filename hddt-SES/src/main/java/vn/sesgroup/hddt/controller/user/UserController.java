...
public class UserController extends AbstractController {
	...
	private int kt = 0;
	@GetMapping("/init")
	public String createcaptcha(Users users, LoginRes us, HttpServletRequest req, Locale locale, String action
		...
		us = cup.getLoginRes();
		req.setAttribute("userName", us.getUserName());
			...
			if(captchaSession.equals(captcha)) {
				...
				if (!checkPassword) {
					...
					if (!pw1.equals(pw2)) {
						...
						MsgRsp rsp = restAPI.callAPINormal("/main/crud", cup.getLoginRes().getToken(), HttpMethod.POST, root);
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id,
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/main/detail/" + _id, cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if (rspStatus.getErrorCode() == 0) {
