...
@Controller
@RequestMapping("/ca_invoice")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class CAInvoiceController extends AbstractController {
	private static final Logger log = LogManager.getLogger(CAInvoiceController.class);
	@Autowired
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception {
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Danh sách ca invoice");
		...
		req.setAttribute("ToDate", commons.convertLocalDateTimeToString(now.plusMonths(CKS_EXPIRES_), Constants.FORMAT_DATE.FORMAT_DATE_WEB));
		req.setAttribute("FromDate", commons.convertLocalDateTimeToString(now, Constants.FORMAT_DATE.FORMAT_DATE_WEB));

		
		if ("true".equals(isroot) && issu.isRoot() == true && issu.isAdmin() == true) {
			return "/issu/ca-invoice";
		} else {
			return "/admin/admin";
		}
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception {
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/ca_invoice/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if (rspStatus.getErrorCode() == 0) {
