...
@Controller
@RequestMapping("/tkdshdonPXK")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class TKDSHDonPXKNBoController extends AbstractController{
	private static final Logger log = LogManager.getLogger(TKDSHDonPXKNBoController.class);
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tkdshdonPXK/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
	...
	public void exportExcel(Locale locale, HttpServletRequest req, HttpServletResponse resp, HttpSession session
		...
		try {
			...
			String url = "/tkdshdonPXK/export-excel-to-fast";
			if("export-excel-general".equals(method))
				url = "/tkdshdonPXK/export-excel-general";
			else if("export-excel-dshdon-ctiet".equals(method))
				url = "/tkdshdonPXK/export-excel-dshdon-ctiet";
			vn.sesgroup.hddt.dto.FileInfo fileInfo = restAPI.callAPIGetFileInfo(url, cup.getLoginRes().getToken(), HttpMethod.POST, root);
			if(null != fileInfo) {
				String fileNameOut = "DANH-SACH-HDPXKNB-DANG-SD.xlsx";
				if("export-excel-general".equals(method))
					fileNameOut = "Thong-ke-tong-quat_PXK.xlsx";
				else if("export-excel-dshdon-ctiet".equals(method))
					fileNameOut = "Thong ke chi tiet_PXK.xlsx";
