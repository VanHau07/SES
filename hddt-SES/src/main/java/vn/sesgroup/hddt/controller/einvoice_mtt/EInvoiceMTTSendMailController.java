...
@Controller
@RequestMapping({
	"/einvoice_mtt-send-mail"
})
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class EInvoiceMTTSendMailController extends AbstractController{
	private static final Logger log = LogManager.getLogger(EInvoiceMTTSendMailController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		return "einvoice_mtt/einvoice_mtt-send-mail";
	}
	...
	private void inquiry(CurrentUserProfile cup, Locale locale, HttpServletRequest req, HttpSession session, String _id , String action) throws Exception{
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			String _tmp = "";
			title = ii.getTaxCode() + " " + ii.getName() + " Thông báo phát hành Hóa đơn điện tử từ máy tính tiền";
