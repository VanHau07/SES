...
public class EinvoiceImportController extends AbstractController{
	private static final Logger log = LogManager.getLogger(EinvoiceImportController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		return "einvoice/einvoice-import";
	}
	...
	public BaseDTO execImportData(HttpServletRequest req, HttpSession session
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/einvoice/import-data", cup.getLoginRes().getToken(), HttpMethod.POST, root);
