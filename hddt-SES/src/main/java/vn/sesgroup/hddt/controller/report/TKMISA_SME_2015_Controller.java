...
@Controller
@RequestMapping("/misaSME2015")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class TKMISA_SME_2015_Controller extends AbstractController{
	private static final Logger log = LogManager.getLogger(TKMISA_SME_2015_Controller.class);
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Thống kê VAT theo chuẩn MISA SME 2015");
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/misaSME2015/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
	...
	@RequestMapping(
			value = {
				 "/export-excel-vat/{token-transaction}"
			}, 
