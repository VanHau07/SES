...
public class HDSDController extends AbstractController {
	@Autowired
	...
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception {
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Danh sách hướng dẫn sử dụng");
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/hdsd/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
