...
public class TBHDSSotSendMailController extends AbstractController{
	private static final Logger log = LogManager.getLogger(TBHDSSotSendMailController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		return "einvoice/tbhdssot-send-mail";
	}
	...
	public BaseDTO execSendMail(HttpServletRequest req, HttpSession session
		...
		msg.setObjData(hData);
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/tbhdssot/send-mail", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		MspResponseStatus rspStatus = rsp.getResponseStatus();
		if(rspStatus.getErrorCode() == 0) {
