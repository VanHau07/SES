...
@Controller
@RequestMapping("/ql-cqt")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class CQTController extends AbstractController{
	private static final Logger log = LogManager.getLogger(CQTController.class);
	@Autowired RestAPIUtility restAPI; 
	
	private String _id;
	private String code;
	private String name;
	private String codetinh;
	
	
	@RequestMapping(value = "/init", method = {RequestMethod.POST, RequestMethod.GET})
	public String init(Locale locale, Principal principal, HttpServletRequest req) throws Exception{
		req.setAttribute("_TitleView_", Constants.PREFIX_TITLE + " - Danh sách cơ quan thuế");
		return "setting-admin/cqt";
	}
	...
	private BaseDTO checkDataSearch(Locale locale, HttpServletRequest req, HttpSession session) {
		...
		codetinh = commons.getParameterFromRequest(req, "codetinh");
		return dto;
	}
	...
	public BaseDTO execSearch(Locale locale, HttpServletRequest req, HttpSession session) throws Exception{
		...
		hData.put("code", code);
		hData.put("name", name);
		hData.put("codett", codetinh);
		msg.setObjData(hData);
		
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/ql-cqt/list", cup.getLoginRes().getToken(), HttpMethod.POST, root);
		...
		if(rspStatus.getErrorCode() == 0) {
			...
			if(!jsonData.at("/rows").isMissingNode()) {
				...
				for(JsonNode row: rows) {
					...
					hItem.put("CodeTinh", commons.getTextJsonNode(row.at("/codett")));
