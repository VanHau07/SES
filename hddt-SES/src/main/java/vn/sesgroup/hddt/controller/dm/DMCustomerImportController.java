...
public class DMCustomerImportController extends AbstractController{
	private static final Logger log = LogManager.getLogger(DMCustomerImportController.class);
	...
	public String init(Locale locale, HttpServletRequest req, HttpSession session
		...
		req.setAttribute("_header_", "Import khách hàng");
		LoadParameter(cup, locale, req, action);
		return "dm/customer-import";
	}
	...
	public BaseDTO execImportData(HttpServletRequest req, HttpSession session
		...
		JSONRoot root = new JSONRoot(msg);
		MsgRsp rsp = restAPI.callAPINormal("/dmcustomer/import-data", cup.getLoginRes().getToken(), HttpMethod.POST, root);
