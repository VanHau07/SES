$(function(){
	...
	dateInputFormat($('#f-einvoice_mtt').find('#to-date'));
	...
	_gridMain.kendoGrid({
		dataSource: new kendo.data.DataSource({
			transport: {
				read: {
					type: 'POST',
					url: ROOT_PATH + '/main/tkdsmtt/search',
                    dataType: 'json',
	...
	$("#f-einvoice_mtt").find('button[data-action]').click(function (event) {
		...
		case 'export-excel-general':	
			...
			$.ajax({
				type: "POST",
				datatype: "json",
				url: ROOT_PATH + '/main/tkdsmtt/check-data-export',
				data: objData,
				beforeSend: function(req) {
				...
				success:function(res) {
					...
					if(res.errorCode == 0) {
						...
						if(dataAction == 'export-excel-fast')
							window.open(ROOT_PATH + '/main/tkdsmtt/export-excel-to-fast/' + tokenTransaction,'_blank');
						else if(dataAction == 'export-excel-general')
							window.open(ROOT_PATH + '/main/tkdsmtt/export-excel-general/' + tokenTransaction,'_blank');
						else
							window.open(ROOT_PATH + '/main/tkdsmtt/export-excel-dshdon-ctiet/' + tokenTransaction,'_blank');
					}else{
...
function getDataSearch(){
	...
	dataPost['from-date'] = $('#f-einvoice_mtt #from-date').val() == null? '': $('#f-einvoice_mtt #from-date').val();
	dataPost['to-date'] = $('#f-einvoice_mtt #to-date').val() == null? '': $('#f-einvoice_mtt #to-date').val();
	dataPost['nmua-mst'] = $('#f-einvoice_mtt #nmua-mst').val() == null? '': $('#f-einvoice_mtt #nmua-mst').val();
	dataPost['nmua-ten'] = $('#f-einvoice_mtt #nmua-ten').val() == null? '': $('#f-einvoice_mtt #nmua-ten').val();
