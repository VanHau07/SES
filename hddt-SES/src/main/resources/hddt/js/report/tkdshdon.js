	...
	_gridMain.kendoGrid({
		dataSource: new kendo.data.DataSource({
			transport: {
				read: {
					type: 'POST',
					url: ROOT_PATH + '/main/tkdshdon/search',
                    dataType: 'json',
	...
	$("#f-einvoices").find('button[data-action]').click(function (event) {
		...
		case 'export-excel-general':	
			...
			$.ajax({
				type: "POST",
				datatype: "json",
				url: ROOT_PATH + '/main/tkdshdon/check-data-export',
				data: objData,
				beforeSend: function(req) {
				...
				success:function(res) {
					...
					if(res.errorCode == 0) {
						...
						if(dataAction == 'export-excel-fast')
							window.open(ROOT_PATH + '/main/tkdshdon/export-excel-to-fast/' + tokenTransaction,'_blank');
						else if(dataAction == 'export-excel-general')
							window.open(ROOT_PATH + '/main/tkdshdon/export-excel-general/' + tokenTransaction,'_blank');
						else
							window.open(ROOT_PATH + '/main/tkdshdon/export-excel-dshdon-ctiet/' + tokenTransaction,'_blank');
					}else{
