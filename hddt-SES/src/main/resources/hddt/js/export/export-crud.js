$(function(){
		if(vIsEdit){
			initInputNumber('#f-export-crud .text-number');
			dateInputFormat($('#f-export-crud').find('#ngay-lap'));
		}
...
//					, template: '<i class="mdi mdi-close-box fs-25 text-black"></i>'
			...
			dataBound: function(e) {
				initInputNumber('#f-export-crud #grid .input-grid-number');
...
//					console.log(code)
		...
		$('#f-export-crud').find('button[data-action]').click(function (event) {
			event.preventDefault();/*event.stopPropagation();*/
			...
			case 'back':
				...
				try{
					if($('#f-export').find('#grid').length > 0)
						$('#f-export').find('#grid').data("kendoGrid").dataSource.read();
				}catch(err){}
				break;
			case 'sign':
				objDataSend['_id'] = $('#f-export-crud').find('input[name="_id"]').val();
				$.ajax({
					type: "POST",
					datatype: "json",
...
//												        	hideLoading();
																			...
																			xhrSign.onreadystatechange = function(e) {
																				if(4 == this.readyState && this.status == 200) {
																					...
																					if(res){
																						if(res.errorCode == 0) { 
																							hideLoading();
																							$('#f-export-crud').find('button[data-action="back"]').trigger('click');
																						}else{
	...
	function calcAmountInGrid($obj){
		...
		var _val = '';
		var _numeral = null;
		var loai_tien_tt = $('#f-export-crud').find('#loai-tien-tt').val();
	...
	function calcTotalAmount(){
		...
		$('#f-export-crud').find('#tong-tien-truoc-thue').val(sumAmount.toFixed(4))
		FormatCurrency($('#f-export-crud').find('#tong-tien-truoc-thue')[0], loai_tien_tt);
		$('#f-export-crud').find('#tong-tien-thue-gtgt').val(sumAmountVAT.toFixed(4))
		FormatCurrency($('#f-export-crud').find('#tong-tien-thue-gtgt')[0], loai_tien_tt);
		$('#f-export-crud').find('#tong-tien-da-co-thue').val(sumAmountAfterTax.toFixed(4))
		FormatCurrency($('#f-export-crud').find('#tong-tien-da-co-thue')[0], loai_tien_tt);
		
		$('#f-export-crud').find('#tien-bang-chu').val(readMoneyInWords(sumAmountAfterTax.toFixed(4), loai_tien_tt));
	}
	...
	function getDataToSave(){
		...
		dataPost['_id'] = $('#f-export-crud').find('input[name="_id"]').val();
		dataPost['mau-so-hdon'] = $('#f-export-crud').find('#mau-so-hdon').val();	
		dataPost['ten-loai-hd'] = $('#f-export-crud').find('#ten-loai-hd').val();
		dataPost['hinh-thuc-thanh-toan'] = $('#f-export-crud').find('#hinh-thuc-thanh-toan').val();
		dataPost['hinh-thuc-thanh-toan-text'] = $('#f-export-crud').find('#hinh-thuc-thanh-toan').find('option:selected').text();
		dataPost['ngay-lap'] = $('#f-export-crud').find('#ngay-lap').val();
		
		dataPost['kh-mst'] = $('#f-export-crud').find('#kh-mst').val();
		dataPost['kh-ho-ten-ng-xuat'] = $('#f-export-crud').find('#kh-ho-ten-ng-xuat').val();
		
		dataPost['nban-dchi'] = $('#f-export-crud').find('#nban-dchi').val();
		
		dataPost['kh-ho-ten-ng-vc'] = $('#f-export-crud').find('#kh-ho-ten-ng-vc').val();
		dataPost['kh-ho-ten-ng-dd'] = $('#f-export-crud').find('#kh-ho-ten-ng-dd').val();
		dataPost['kh-ve-viec'] = $('#f-export-crud').find('#kh-ve-viec').val();
		dataPost['kh-ten-don-vi'] = $('#f-export-crud').find('#kh-ten-don-vi').val();
		dataPost['kh-dia-chi'] = $('#f-export-crud').find('#kh-dia-chi').val();
		
		dataPost['kh-lenh-dd-noi-bo'] = $('#f-export-crud').find('#kh-lenh-dd-noi-bo').val();
		dataPost['kh-pt-vc'] = $('#f-export-crud').find('#kh-pt-vc').val();
		dataPost['kh-email'] = $('#f-export-crud').find('#kh-email').val();
		dataPost['kh-so-dt'] = $('#f-export-crud').find('#kh-so-dt').val();
		dataPost['kh-so-tk'] = $('#f-export-crud').find('#kh-so-tk').val();
		dataPost['kh-tk-tai-ngan-hang'] = $('#f-export-crud').find('#kh-tk-tai-ngan-hang').val();
		
		dataPost['tong-tien-truoc-thue'] = $('#f-export-crud').find('#tong-tien-truoc-thue').val();
		dataPost['loai-tien-tt'] = $('#f-export-crud').find('#loai-tien-tt').val();
		dataPost['ty-gia'] = $('#f-export-crud').find('#ty-gia').val();
		dataPost['tong-tien-thue-gtgt'] = $('#f-export-crud').find('#tong-tien-thue-gtgt').val();
		dataPost['tong-tien-da-co-thue'] = $('#f-export-crud').find('#tong-tien-da-co-thue').val();
		dataPost['tien-bang-chu'] = $('#f-export-crud').find('#tien-bang-chu').val();
