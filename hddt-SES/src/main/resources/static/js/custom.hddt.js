...
function readMoneyInWordsOthers(SoTien, LoaiTien ){
//	var n = numeral(SoTien);
//	return readMoneyDBLInWords(n.value() == null? 0: n.value(), LoaiTien);
	var value = 0;
	var valueDecimal = 0;
...
//	var decimalPart = value.toString().substring(value.toString().indexOf('.') + 1);		
//	valueDecimal = decimalPart;		
//		valueDecimal = valueDecimal.toFixed(6);	
		var parts = value.toString().split(".");
		var valueDecimal = parts[1];	
	...
	if(valueDecimal > 0) {
		var posDot = r.lastIndexOf(".");
		if(posDot == r.length -1) r = r.substr(0, posDot);
		r += ' và ' + DocTienBangChu(valueDecimal,'cents');
...
function readMoneyInWordsOthersV2(SoTien, LoaiTien, decimalPlaces) {
  var value = 0;
  var valueDecimal = 0;
  var n = numeral(SoTien);
  ...
  if ('VND' == LoaiTien) {
    ...
    var parts = value.toString().split(".");
    valueDecimal = parts[1];
    if (valueDecimal && decimalPlaces) {
      valueDecimal = valueDecimal.padEnd(decimalPlaces, '0').substring(0, decimalPlaces);
    }
  }
  ...
  if (valueDecimal > 0) {
    var posDot = r.lastIndexOf(".");
    if (posDot == r.length - 1) r = r.substr(0, posDot);
    r += ' và ' + convertNumberToWords(valueDecimal, 'cents');
  }
...
function convertNumberToWords(number, donvi) {
  const units = ['', 'một', 'hai', 'ba', 'bốn', 'năm', 'sáu', 'bảy', 'tám', 'chín', 'mười', 'mười một', 'mười hai', 'mười ba', 'mười bốn', 'mười năm', 'mười sáu', 'mười bảy', 'mười tám', 'mười chín'];
  ...
  while (number > 0) {
    ...
    if (part !== 0) {
      ...
      if (tens === 0 && unitsDigit === 0) {
        partWords += 'linh ';
      } else if (tens === 1 && unitsDigit > 0) {
        partWords += units[tens * 10 + unitsDigit] + ' ';
      } else if (tens > 1) {
      ...
      words = partWords + powersOfTen[powerIndex] + ' ' + words;
    }
...
//		emptyInputBehavior: "zero",
	...
	const anElement = AutoNumeric.multiple($input, null, {
		...
		decimalPlaces: 8,					//Defines the default number of decimal places to show on the formatted value, and keep for the precision: default = 2
