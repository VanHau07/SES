...
import vn.sesgroup.hddt.user.dao.ConfigEmailServerAdminDAO;
import vn.sesgroup.hddt.utility.Constants;
import vn.sesgroup.hddt.utility.Json;

@Repository
@Transactional
public class ConfigEmailServerAdminImpl extends AbstractDAO implements ConfigEmailServerAdminDAO{
	private static final Logger log = LogManager.getLogger(ConfigEmailServerAdminImpl.class);
	...
	public MsgRsp crud(JSONRoot jsonRoot) throws Exception {
		...
		MongoCollection<Document> collection = mongoClient.getDatabase(cfg.dbName).getCollection("ConfigEmailAdmin");
		      try {
		...
		if(docTmp == null) {		//THEM MOI
			...
			collection = mongoClient.getDatabase(cfg.dbName).getCollection("ConfigEmailAdmin");
			...
			collection = mongoClient1.getDatabase(cfg.dbName).getCollection("ConfigEmailAdmin");
