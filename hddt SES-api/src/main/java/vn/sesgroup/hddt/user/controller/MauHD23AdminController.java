...
@RestController
@RequestMapping(value = "/mauhd_admin")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class MauHD23AdminController {
	Commons commons = new Commons();
	@Autowired private MauHD23AdminDAO dao;
	...
	@RequestMapping(value = "/updateDBTheoNam", method = RequestMethod.POST,
			consumes = {MediaType.APPLICATION_JSON_VALUE},		//MediaType.TEXT_PLAIN_VALUE, 
			produces = {MediaType.APPLICATION_JSON_VALUE})
	public ResponseEntity<?> updateDBTheoNam(@RequestBody JSONRoot jsonRoot) throws Exception{
		MsgRsp rsp = dao.updateDBTheoNam(jsonRoot);
		HttpHeaders headers = new HttpHeaders();
		headers.add(HttpHeaders.CONTENT_TYPE, "application/json; charset=UTF-8");
		return ResponseEntity.ok()
				.headers(headers)
