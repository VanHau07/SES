...
import vn.sesgroup.hddt.user.dao.QLCQTDao;
...
@Repository
@Transactional
public class QLCQTImpl extends AbstractDAO implements QLCQTDao {
	private static final Logger log = LogManager.getLogger(QLCQTImpl.class);
	...
	public MsgRsp crud(JSONRoot jsonRoot) throws Exception {
		...
		String codett = commons.getTextJsonNode(jsonData.at("/codett")).trim().replaceAll("\\s+", " ");
...
//////////////////////////////////////////////////////////////////////////////
		...
		case Constants.MSG_ACTION_CODE.CREATED:
			...
			MongoCollection<Document> collection = mongoClient.getDatabase(cfg.dbName).getCollection("DMChiCucThue");
			      try {
			...
			/* LUU DU LIEU HD */
			docUpsert = new Document("code", code).append("name", name).append("IsDelete", false).append("tinhthanh_ma", codett);
			/* END - LUU DU LIEU HD */
		
			mongoClient = cfg.mongoClient();
			collection = mongoClient.getDatabase(cfg.dbName).getCollection("DMChiCucThue");
		...
		case Constants.MSG_ACTION_CODE.MODIFY:
			...
			pipeline.add(
					new Document("$lookup", 
						...
						.append("as", "DMChiCucThue")
					)
				);
				pipeline.add(new Document("$unwind", new Document("path", "$DMChiCucThue").append("preserveNullAndEmptyArrays", true)));
			...
			if (null == docTmp) {
				...
				return rsp;
					...
					collection.findOneAndUpdate(docFind,
							new Document("$set", 
									new Document("name", name).append("code", code).append("tinhthanh_ma", codett)		
							),
						options
					);
					  mongoClient.close();
	...
	public MsgRsp list(JSONRoot jsonRoot) throws Exception {
		...
		if(objData != null) {
			jsonData = Json.serializer().nodeFromObject(objData);	
			 ...
			 codett = commons.getTextJsonNode(jsonData.at("/codett")).replaceAll("\\s", "");
		...
		if(!"".equals(codett))
			docMatch.append("tinhthanh_ma", commons.regexEscapeForMongoQuery(codett));
		if(!"".equals(name))
		...
		if(null != rows) {
			for(Document doc: rows) {
				...
				hItem.put("codett", doc.get("tinhthanh_ma"));
