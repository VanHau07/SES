...
public class StatisticReportController {
	...
	@Autowired private StatisticReportDAO dao;
	...
	@RequestMapping(value = "/backupData", method = RequestMethod.POST,
			consumes = {MediaType.APPLICATION_JSON_VALUE},		//MediaType.TEXT_PLAIN_VALUE, 
			produces = {MediaType.APPLICATION_JSON_VALUE})
	public ResponseEntity<?> backupData(@RequestBody JSONRoot jsonRoot) throws Exception{
		MsgRsp rsp = dao.backupData(jsonRoot);
		HttpHeaders headers = new HttpHeaders();
		headers.add(HttpHeaders.CONTENT_TYPE, "application/json; charset=UTF-8");
		return ResponseEntity.ok()
				.headers(headers)
