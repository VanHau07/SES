...
@RestController
@RequestMapping(value = "/phoihd")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class QLPhoiHDonController {
	Commons commons = new Commons();
	@Autowired private PhoiHDonDao dao;
	...
	@RequestMapping(value = "/viewimg/{_id}", method = RequestMethod.POST,
			consumes = {MediaType.APPLICATION_JSON_VALUE},		//MediaType.TEXT_PLAIN_VALUE, 
			produces = {MediaType.APPLICATION_JSON_VALUE})
	public ResponseEntity<?> viewimg(@RequestBody JSONRoot jsonRoot
			, @PathVariable(name = "_id", required = false) String _id) throws Exception{
		MsgRsp rsp = dao.viewimg(jsonRoot, _id);
		HttpHeaders headers = new HttpHeaders();
		headers.add(HttpHeaders.CONTENT_TYPE, "application/json; charset=UTF-8");
		return ResponseEntity.ok()
				.headers(headers)
