...
import vn.sesgroup.hddt.user.dao.QLTinhThanhDao;
...
@Repository
@Transactional
public class QLTinhThanhImpl extends AbstractDAO implements QLTinhThanhDao {
	private static final Logger log = LogManager.getLogger(QLTinhThanhImpl.class);
...
//////////////////////////////////////////////////////////////////////////////
		...
		case Constants.MSG_ACTION_CODE.CREATED:
			...
			MongoCollection<Document> collection = mongoClient.getDatabase(cfg.dbName).getCollection("DMTinhThanh");
			      try {
			...
			/* LUU DU LIEU HD */
			docUpsert = new Document("code", code).append("name", name).append("IsDelete", false);
			/* END - LUU DU LIEU HD */
			
			mongoClient = cfg.mongoClient();
			collection = mongoClient.getDatabase(cfg.dbName).getCollection("DMTinhThanh");
		...
		case Constants.MSG_ACTION_CODE.MODIFY:
			...
			pipeline.add(
					new Document("$lookup", 
						...
						.append("as", "DMTinhThanh")
					)
				);
				pipeline.add(new Document("$unwind", new Document("path", "$DMTinhThanh").append("preserveNullAndEmptyArrays", true)));				
