...
import vn.sesgroup.hddt.user.dao.RoleManagerUserAdminDAO;
...
@Repository
@Transactional
public class RoleManagerUserAdminImpl extends AbstractDAO implements RoleManagerUserAdminDAO {
	private static final Logger log = LogManager.getLogger(RoleManagerUserAdminImpl.class);
...
//												,
												new Document("$eq", Arrays.asList("$k", "FULL-RIGHT-ADMIN-MANAGER"))
											)
										)
									)
								)
...
//												,
				...
				mongoClient = cfg.mongoClient();
				collection = mongoClient.getDatabase(cfg.dbName).getCollection("RolesRightManage");
				collection.findOneAndUpdate(new Document("IssuerId", header.getIssuerId()).append("_id", objectId)
						, new Document("$set", 
