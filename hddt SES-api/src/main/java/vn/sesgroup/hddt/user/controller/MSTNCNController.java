...
@RestController
@RequestMapping(value = "/mstncn")
@Scope(value = WebApplicationContext.SCOPE_REQUEST)
public class MSTNCNController {
	Commons commons = new Commons();
	@Autowired private MSTNCNDao dao;
	...
	@RequestMapping(value = "/viewimg/{phoi}", method = RequestMethod.POST,
			consumes = {MediaType.APPLICATION_JSON_VALUE},		//MediaType.TEXT_PLAIN_VALUE, 
			produces = {MediaType.APPLICATION_JSON_VALUE})
	public ResponseEntity<?> viewimg(@RequestBody JSONRoot jsonRoot
			, @PathVariable(name = "phoi", required = false) String phoi) throws Exception{
		MsgRsp rsp = dao.viewimg(jsonRoot, phoi);
		HttpHeaders headers = new HttpHeaders();
		headers.add(HttpHeaders.CONTENT_TYPE, "application/json; charset=UTF-8");
		return ResponseEntity.ok()
				.headers(headers)
