...
import vn.sesgroup.hddt.user.dao.QLMauSoTKhaiDao;
...
@Repository
@Transactional
public class QLMauSoTKhaiImpl extends AbstractDAO implements QLMauSoTKhaiDao {
	private static final Logger log = LogManager.getLogger(QLMauSoTKhaiImpl.class);
...
//////////////////////////////////////////////////////////////////////////////
		...
		case Constants.MSG_ACTION_CODE.CREATED:
			...
			MongoCollection<Document> collection = mongoClient.getDatabase(cfg.dbName).getCollection("DMMSTKhai");
			      try {
			...
			/* LUU DU LIEU HD */
			docUpsert = new Document("name", name).append("code", name).append("IsDelete", false);
			/* END - LUU DU LIEU HD */
			
			mongoClient = cfg.mongoClient();
			collection = mongoClient.getDatabase(cfg.dbName).getCollection("DMMSTKhai");
...
////////////////////////////////////////////////////////////////////////////////////////
		case Constants.MSG_ACTION_CODE.MODIFY:
			...
			pipeline.add(
				...
				pipeline.add(new Document("$unwind", new Document("path", "$DMMSTKhai").append("preserveNullAndEmptyArrays", true)));
			...
			if (null == docTmp) {
				...
				return rsp;
					...
					collection.findOneAndUpdate(docFind,
							new Document("$set", 
									new Document("name", name).append("code", name)
							),
						options
					);
					  mongoClient.close();
