...
import vn.sesgroup.hddt.user.dao.CreateUserAdminDAO;
...
@Repository
@Transactional
public class CreateUserAdminImpl extends AbstractDAO implements CreateUserAdminDAO {
	private static final Logger log = LogManager.getLogger(CreateUserAdminImpl.class);
								...
								new Document("$expr", 
									new Document("$and", 
										Arrays.asList(
											...
											, new Document("$ne", Arrays.asList("$IsAdmin", false))
										)
									)
								)
							)
								...
								new Document("$expr", 
									new Document("$and", 
										Arrays.asList(
											...
											, new Document("$ne", Arrays.asList("$IsDelete", true))
											, new Document("$eq", Arrays.asList("$IsRoleRoot", true))
										)
									)
								)
							)
...
//				mongoTemplate.getCollection("LogChangedPassword").insertOne(
//						new Document("UserID", _id)
//							//.append("UserType", Constants.USER_TYPE.ADMIN)	//ADMIN; USER
//							.append("OldPassword", oldPasswordInSystem)
//							.append("IPAddress", header.getIpAddress())
//							//.append("ChangedFrom", Constants.USER_CHANGED_FROM.PASSWORD_ADMIN_RESET)
//							.append("ChangedDate", LocalDateTime.now())
//							.append("InfoReset", 
//									new Document("ResetDate", LocalDateTime.now())
//										.append("ResetUserID", header.getUserId())
//										.append("ResetUserName", header.getUserName())
//										.append("ResetUserFullName", header.getUserFullName())
//									)
//					);
